...
const { env } = require('@nextcms/utils');
...
const createUpdateNotifier = nextcms => {
  let config = null;
  ...
  try {
    config = new Configstore(
      pkg.name,
      {},
      { configPath: path.join(nextcms.dirs.root, '.nextcms-updater.json') }
    );
  } catch {
  ...
  return {
    notify({ checkInterval = CHECK_INTERVAL, notifInterval = NOTIF_INTERVAL } = {}) {
      if (env.bool('NEXTCMS_DISABLE_UPDATE_NOTIFICATION', false) || !config) {
        return;
      }
